...
function trataHora($n){
	...
	function agendaHora($date, $hora){
		//COUNT() -> função nativa do MySQL que retorna o nro de linhas que atendem a cláusula
		$query = ("SELECT COUNT(*) as total FROM agenda WHERE date LIKE '".$date."' AND hora LIKE '".$hora."'");
		$sql = mysqli_query($con, $query);
		$result = mysqli_fetch_array($sql);
		$total = $result['total'];
